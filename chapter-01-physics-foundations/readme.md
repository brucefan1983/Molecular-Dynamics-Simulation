...
量子力学的基本假设之一：微观系统状态的描述与量子态叠加原理


在量子力学中，一个系统（一个或多个“粒子”组成的整体）在某一时刻的状态由某个Hilbert空间中的一个矢量描述。而且规定：相差一个复数因子的两个矢量描写同一个状态。我们会常称表述状态的矢量为“状态矢量”，或者“态矢”，或者就简单地说“态”。一般地，我们记一个态矢为 $|\psi\rangle$ 和 $|\phi\rangle$ 等等。

我们知道，Hilbert空间中的矢量可以被数乘，还可以相互线性组合。上面的规定告诉我们，态矢被数乘之后还是原来的态矢，并不是一个新的态矢。虽然数乘不给出新的态矢，但线性组合能给出新的态矢。例如，如果 $|\psi_1\rangle$ 和 $|\psi_2\rangle$ 是不同的态矢，那么当 $a$ 和 $b$ 都不为零时，$a |\psi_1\rangle + b |\psi_2\rangle$ 是一个不同于 $|\psi_1\rangle$ 也不同于 $|\psi_2\rangle$ 的新态矢。如果记这个新的态矢为 $|\psi\rangle$ ，那么我们可以将这个线性组合的规则写为

$$
|\psi\rangle = a |\psi_1\rangle + b |\psi_2\rangle.
$$

这个线性组合用物理的语言来说叫做“线性叠加”（linear superposition）。系数 $a$ 和 $b$ 叫做叠加系数。量子态的“线性叠加原理”指的就是：两个不同的态矢的线性组合给出一个新的态矢（如果叠加系数都不为零的话）。

注意，根据之前的规定，若 $|\psi_1\rangle$ 和 $|\psi_2\rangle$ 不是不同的态矢，即 $|\psi_1\rangle = c |\psi_2\rangle$ ，则上式将给出 $|\psi\rangle = (ac+b) |\psi_2\rangle$ ，还是一个与 $|\psi_2\rangle$ 一样的态。所以，量子力学中的叠加原理与经典物理中的叠加原理是很不一样的。例如，在经典力学中，将两个频率、相位、振幅都相等（即“状态”相同）的两列机械波相叠加，将得到一个振幅为两倍的新的机械波。所以，一个量子态完全是由它的“方向”确定的，而它的“大小”（或者“长度”）并不重要。我总可以随意地改变它的“长度”，使其“长度”等于1，即

$$
|||\psi\rangle|| = |||\psi\rangle||^2 = \langle \psi|\psi\rangle = 1.
$$
...
在量子力学中，一个系统的某个物理量由某个Hilbert空间中的厄米算符描述。当系统处于某一个态时，对该物理量测量所得到的值在理论上是不能确切地预言的。量子力学能给出的仅仅是所有可能的测量值的集合，以及得到其中任意一个值的概率。取值集合和相应的概率由如下规则给出。

设考虑的物理量为 $A$ ，它是某个Hilbert空间中的厄米算符。为简单起见，我们暂时假设物理量的本征值谱是离散的。
设在同一个Hilbert空间中系统的态矢为 $|\psi\rangle$ ，而且设态矢 $|\psi\rangle$ 是归一化的，即

$$
\langle \psi | \psi \rangle = 1.
$$

设已经找到了厄米算符 $A$ 的本征值谱 $\{a_i\}$ 和相应的本征矢量（在量子力学中也叫作本征态）$|\psi_i\rangle$ ：

$$
A |\psi_i\rangle = a_i |\psi_i\rangle.
$$

对于一个厄米算符，各个本征值都是实数，而且我们总是可以将它的本征矢量的集合取为正交、归一、完备的基。我们假设已经这样做了。对于这样的基，我们有完备性关系：
...
$$
|\psi\rangle  = I |\psi\rangle = \sum_i |\psi_i\rangle \langle \psi_i | \psi\rangle
= \sum_i |\psi_i\rangle c_i.
$$

其中，我们定义了展开系数

$$
c_i \equiv \langle \psi_i | \psi\rangle.
$$


我们假设，当系统处于态 $|\psi\rangle$ 时测量物理量$A$将得到其本征值谱 $\{a_i\}$ 中的某一个值，其中得到值 $a_i$ 的概率为 $|c_i|^2$ 。我们期望所有可能的概率的和为1，这对于归一化的态矢确实是对的：

$$
1 = \langle \psi | \psi \rangle =
\sum_i \langle \psi |\psi_i\rangle \langle \psi_i | \psi \rangle =
\sum_i \langle \psi_i |\psi\rangle^{\ast} \langle \psi_i | \psi \rangle =
\sum_i c_i^{\ast} c_i = \sum_i|c_i|^2.
$$

一个特殊的情况是，除了某一个系数——比方说 $c_1$ ——不等于零，其它的系数都等于零。此时这个不等于零的系数的模方 $|c_1|^2=1$ ，故可推断，测量物理量$A$将百分之百地得到值 $a_1$ 。这就揭示出本征值和本征态在量子力学中的物理意义：当系统处于某个物理量的某个本征态时，测量该力学量将只可能得到一个值，即与该本征态对应的本征值。

对于一个非本征态 $|\psi\rangle$ ，测量力学量 $A$ 得到值 $a_i$ 的概率为 $|c_i|^2$ ，故可以仿照统计力学中对平均值的定义来定义一个物理量在一个量子态中的期望值（expectation value）：

$$
\langle A \rangle = \sum_i |c_i|^2 a_i.
$$

根据系数 $c_i$ 的定义，我们可以将上式写为

$$
\langle A \rangle = 
\sum_i \langle \psi_i | \psi\rangle^{\ast} \langle \psi_i | \psi\rangle a_i =
\sum_i \langle \psi | \psi_i \rangle \langle \psi_i | \psi\rangle a_i =
\sum_i \langle \psi |A| \psi_i \rangle \langle \psi_i | \psi\rangle
=\langle \psi | A |  \psi\rangle.
$$

在最后一步，我们利用了基的完备性关系。可见，一个物理量在一个态中的期望值就是将力学量算符作用在态矢上再与态矢作内积得到的值。

关于物理量在测量中的取值问题，量子力学还假设，如果在一次测量中，物理量 $A$ 的观测值是一个特定的 $a_n$ ，那么在测量之后，系统就处于物理量 $A$ 的与本征值 $a_n$ 对应的本征态$\psi_n\rangle$ 。如果再一次测量 $A$ 的值，将百分之百地得到 $a_n$ ，因为

$$
\langle \psi_n |A |\psi_n\rangle = a_n \langle \psi_n |\psi_n\rangle = a_n.
$$


量子力学的基本假设之三：基本对易关系

在经典力学中，坐标和动量是两个非常重要的物理量。在量子力学中，这两个量也是非常重要的。上一条假设说，物理量都是（厄米）算符，而我们知道算符不一定是可以交换的，即不一定是对易的。那么，哪些物理量是对易的，哪些是不对易的呢？如果不对易，那么它们的对易子又是多少呢？

通过与经典力学中泊松括号的类比，我们可以假设一个系统中各个坐标 $q_{\alpha}$ 和对应的正则动量 $p_{\alpha}$ 在直角坐标系（时间允许的话以后会讲为什么必须是直角坐标系）中有如下对易关系：
...
其中， $\hbar$ 是约化 Planck 常数，它等于Planck常数 $h$ 除以 $2\pi$ ，即 $\hbar = \frac{h}{2\pi}$ 。在国际单位制中， $h \approx 6.6 \times 10^{-34}$ J s。这些对易关系被称为基本对易关系（fundamental commutation relations）。

下面举两个简单的例子进行说明。

（1）系统中只有一个粒子，它有三个直角坐标$x,y,z$和三个正则动量$p_x, p_y, p_z$。那么，在36中可能的对易子中只有如下三个不等于零的对易子：
\begin{equation}
[x,p_x] = [y,p_y] = [z,p_z] = i \hbar.
\end{equation}

（2）系统中有两个粒子，每个粒子只有一个直角坐标（分别为$x_1$和$x_2$）和一个正则动量（分别为$p_1$ 和$p_2$）。那么，在16种可能的对易子中只有如下两个不等于零的对易子：
\begin{equation}
[x_1,p_1] = [x_2,p_2] = i \hbar.
\end{equation}

现在考虑其它的物理量。量子力学假定，如果在经典力学中有一个物理量是坐标和动量的函数，那么该物理量在量子力学中的算符表示就是将函数中的坐标和动量看成算符即可。下面是几个重要的例子。

（1）Hamilton量算符。在经典力学中，一个三维空间中的粒子的Hamilton量为
\begin{equation}
H(\vect{x}, \vect{p}) = \frac{\vect{p}^2}{2m} + U(\vect{x})
= \frac{p_x^2 + p_y^2 + p_z^2}{2m} + U(x,y,z).
\end{equation}
其中，$U(\vect{x})$是势能。在量子力学中，只要将动量和坐标的每个分量都换成算符，$H(\vect{x}, \vect{p})$也就成了算符。要注意的是，势能算符$U(\vect{x})$是通过将函数$U(\vect{x})$展开为$x,y,z$ 的幂级数来定义的。

（2）角动量。在经典力学中，一个三维空间中的粒子的角动量分量为
\begin{equation}
L_x = y p_z - z p_y;~L_y = z p_x - x p_z;~L_z = x p_y - y p_x.
\end{equation}
在量子力学中，只要将各个坐标和动量分量看成算符，就得到角动量算符。从角动量分量的算符出发，又可以定义角动量平方算符
\begin{equation}
L^2 = \vect{L}^2 = \vect{L} \cdot \vect{L} = L_x^2 + L_y^2 + L_z^2.
\end{equation}


既然物理量都可以看做算符，我们下面就将“物理量的算符”简称为“物理量”。下面利用基本对易关系做几个练习。

\vspace{0.5cm}
\hrule
\hrule
练习。证明：三维空间的自由粒子的Hamilton量与各个动量分量对易。
\hrule
\hrule
\vspace{0.5cm}
...
\hrule
\hrule
练习。证明：三维空间的角动量分量与动量分量之间只有如下三个非零对易子：
\begin{equation}
[L_x, p_y] = i \hbar p_z; ~[L_y, p_z] = i \hbar p_x; ~[L_z, p_x] = i \hbar p_y.
\end{equation}
\hrule
\hrule
\vspace{0.5cm}
...
\hrule
\hrule
练习。证明：三维空间的角动量算符与坐标算符之间只有如下三个非零对易子：
\begin{equation}
[L_x, y] = i \hbar z; ~[L_y, z] = i \hbar x; ~[L_z, x] = i \hbar y.
\end{equation}
\hrule
\hrule
\vspace{0.5cm}
...
我们在数学的学习中证明过，两个厄米算符对易的充要条件是它们有共同的本征矢量。翻译为量子力学的语言，就是说，两个物理量对易的充要条件是它们有共同的本征态。如果两个算符有共同的本征态，那么当系统处于其中一个本征态时，两个力学量的测量值将都是确定的，即百分之百地取某个值。所以，对易的两个物理量是可以同时具有确定的测量值的。

如果两个算符不对易呢？那么它们就不会有共同的本征态，也就不能够同时百分之百地取各自的确定值。所以，不对易的两个物理量是不可以同时具有确定的测量值的。这就是Heisenberg不确定关系。

那么，两个不对易的算符究竟在多大程度上不能同时具有确定的测量值呢？要回答这个问题，我们得先定义描述不确定性的量。这个量的最佳选择就是如下标准差（即方差的平方根）：
\begin{equation}
\Delta A = \sqrt{(\Delta A)^2} =
 \sqrt{\langle (A -\langle A \rangle) ^2\rangle}=
\sqrt{\langle A^2\rangle - \langle A \rangle^2}.
\end{equation}
其中，$\langle A \rangle = \langle \psi |A| \psi \rangle$，$\langle A^2 \rangle = \langle \psi |A^2| \psi \rangle$。可以证明，对任意的两个物理量，有如下不等式：
\begin{equation}
\Delta A \Delta B \geq \frac{1}{2} | \langle [A, B] \rangle |.
\end{equation}
其中，$\langle [A, B] \rangle = \langle \psi |[A, B]| \psi \rangle$。

证明如下。令$\tilde{A} = A -\langle A \rangle$，$\tilde{B} = B -\langle B \rangle$，并设$x$为任意实数，则根据不等式
\begin{equation}
||(\tilde{A} + i x \tilde{B})|\psi\rangle ||^2 \geq 0
\end{equation}
可得
\begin{equation}
(\Delta B)^2 x^2 + (i \langle [A, B] \rangle) x + (\Delta A)^2 \geq 0.
\end{equation}
...
由初中学的一元二次方程的判别式法则可知
\begin{equation}
\Delta A \Delta B \geq \frac{1}{2} \sqrt{-\langle [A, B] \rangle^2}.
\end{equation}
这就是不对易的两个物理量$A$和$B$之间的不确定关系。然而，这个式子还可以稍微换换形式。你有没有觉得奇怪：$\langle [A, B] \rangle^2$一定小于或等于零吗？是的。这是因为，$[A, B]$是反厄米的，故可写为
$[A, B] = i C$，其中$C$是某个厄米算符。于是，$\langle [A, B] \rangle^2 = -\langle C \rangle^2<0$。因为
\begin{equation}
\sqrt{-\langle [A, B] \rangle^2} =
\sqrt{-\langle iC \rangle^2} =
\sqrt{\langle C \rangle^2} =
|\langle C \rangle| =
|\langle iC \rangle| =
|\langle [A, B] \rangle|,
\end{equation}
...
\Delta A \Delta B \geq \frac{1}{2} |\langle [A, B] \rangle|.
\end{equation}
证毕。这个公式就是Heisenberg不确定关系。

将Heisenberg不确定关系运用于坐标和动量，即可知某个方向的坐标$x$与其正则动量$p$有如下不确定关系：
\begin{equation}
\Delta x \Delta p \geq \frac{\hbar}{2}.
\end{equation}


量子力学的基本假设之四：微观系统状态随时间演化的Schrodinger方程

上面只讨论了某一时刻一个系统的状态和物理量的描述，我们还不知道一个系统的状态如何随时间的流逝而演化。一个随时间的流逝而变化的态矢是一个依赖于时间的态矢，可由记号 $|\psi(t)\rangle$ 代表。

量子力学假设，一个系统的态矢 $|\psi(t)\rangle$ 的时间变化规律由Schr\"odinger方程描述：

$$
i\hbar \frac{\partial}{\partial t} |\psi(t)\rangle = H(t) |\psi(t)\rangle.
$$

其中，$H$是系统的Hamilton量，它一般也是依赖于时间的，即“含时”（time-dependenct）的。

粗略地说，Schr\"odinger方程在量子力学中的地位与Newton第二定律在Newton力学中的地位是相当的。Schr\"odinger方程是一阶微分方程，故只需要知道某一时刻系统的状态，就可以确定之后任意时刻系统的状态。

在量子力学中，我们最关心的是物理量的期望值，以及它们随时间的变化规律。如果系统的态矢依赖与时间，那么一个物理量的期望值一般也依赖于时间。我们可以计算期望值的时间变化率：
...
其中，物理量 $A=A(t)$ 一般也是含时的。


利用含时Schr\"odinger方程证明:
...
利用上述期望值的演化方程证明如下Ehrenfest定理

$$
m\frac{d\langle \vect{x} \rangle}{dt}= \langle \vect{p} \rangle,
$$

$$
\frac{d\langle \vect{p} \rangle}{dt} = \langle -\nabla U(\vect{x}) \rangle.
$$

该定理告诉我们：量子力学中的物理量的期望值满足经典力学的运动方程。这是Bohr的对应原理的理论依据之一。
...
如果Hamilton量与时间无关，即$\frac{\partial H}{\partial t} = 0$，我们很自然地会尝试将与时间有关的态矢$|\psi(t)\rangle$写为一个与时间无关的态矢$|\psi\rangle$与一个关于时间的函数$f(t)$的乘积：
\begin{equation}
|\psi(t)\rangle = |\psi\rangle f(t) = f(t) |\psi\rangle.
\end{equation}
下面让我们看看，这样的分解是否一定正确？将该表达式代入前面的Schr\"odinger方程可得
\begin{equation}
i\hbar \frac{\partial f(t)}{\partial t}  |\psi \rangle = f(t) H |\psi\rangle.
\end{equation}
方程右边出现了Hamilton量对不含时的态矢的作用。一般来说，不含时的态矢不是不含时的Hamilton量的本征矢量，但假若我们求出了Hamilton量的本征值谱和相应的正交归一完备的基矢量：
\begin{equation}
H |\psi_n \rangle = E_n |\psi_n \rangle,
\end{equation}
那么总可以将态矢$|\psi\rangle$按这个基展开
\begin{equation}
|\psi \rangle = \sum_n |\psi_n \rangle \langle \psi_n | \psi\rangle
\equiv \sum_n |\psi_n \rangle c_n = \sum_n c_n |\psi_n \rangle.
\end{equation}
于是，我们有
\begin{equation}
i\hbar \frac{\partial f(t)}{\partial t}
\sum_n |\psi_n \rangle \langle \psi_n | \psi\rangle
= f(t) H \sum_n |\psi_n \rangle \langle \psi_n | \psi\rangle
= f(t) \sum_n E_n |\psi_n \rangle \langle \psi_n | \psi\rangle.
\end{equation}
因为这组基是正交完备的，必须（为什么？）对每一个$n$，都有（我们给函数$f(t)$加了个下标来表示下面的方程是针对$n$的）：
\begin{equation}
i\hbar \frac{\partial f_n(t)}{\partial t}
= f_n(t)  E_n .
\end{equation}
该方程的通解解为$f_n(t) \propto e^{-\frac{i}{\hbar}E_n t}$。如果我们要求$|\psi(t)\rangle$和$|\psi\rangle$都是归一化的话，那么函数$f(t)$是一个模为1的复值函数（即一个相因子函数），故有
\begin{equation}
 f_n(t) = e^{-\frac{i}{\hbar}E_n t} e^{i \theta_n}.
\end{equation}
其中，$\theta_n$为$t=0$时的相位。

现在麻烦来了。我们一开始假设只有一个时间函数$f(t)$，现在却解出不止一个。这就说明一般情况下不能将含时的态矢写成一个不含时的态矢与一个时间函数的乘积。然而，通过上面的推导，我们可以猜出含时态矢的正确表达式：
\begin{equation}
|\psi(t)\rangle = \sum_n c_n |\psi_n \rangle e^{-\frac{i}{\hbar} E_n t } e^{i \theta_n}.
\end{equation}
如果要求$|\psi(t=0)\rangle = |\psi\rangle$，就有$\theta_n=0$，从而可以将上式简化：
\begin{equation}
|\psi(t)\rangle = \sum_n c_n |\psi_n \rangle e^{-\frac{i}{\hbar} E_n t } .
\end{equation}

\vspace{0.5cm}
\hrule
\hrule
练习。验证上面最后给出的含时态矢满足含时Schr\"odinger方程。
\hrule
\hrule
\vspace{0.5cm}

可见，求解一个含时Schr\"odinger方程的任务就归结为求解如下本征方程：
\begin{equation}
H |\psi\rangle = E |\psi\rangle.
\end{equation}
这个方程叫做不含时（time-independent）Schr\"odinger方程，但因为我们常常要求解的就是这个方程（而不是上面的含时Schr\"odinger方程），故通常就把不含时Schr\"odinger方程简称为Schr\"odinger方程。


为了强调这个结果，我们将上面的公式用稍微不同的方式再写一遍：
\begin{equation}
|\psi(t)\rangle = \sum_n c_n e^{-\frac{i}{\hbar} E_n t }|\psi_n \rangle.
\end{equation}
因为$|\psi_n\rangle$是$H$的本征态，那么$|\psi_n\rangle$也是$e^{-\frac{i}{\hbar} H t }$的本征态，本征值是$e^{-\frac{i}{\hbar} E_n t }$。于是，我们可以将含时态矢写成如下形式：
\begin{equation}
|\psi(t)\rangle = \sum_n c_n e^{-\frac{i}{\hbar} H t }|\psi_n \rangle=
e^{-\frac{i}{\hbar} H t } \sum_n c_n |\psi_n \rangle=
e^{-\frac{i}{\hbar} H t } |\psi(t=0) \rangle.
\end{equation}
这就是说，将算符$e^{-\frac{i}{\hbar} H t }$作用在$t= 0$时刻的态矢便可得到$t$时刻的态矢。这个算符是个幺正算符，叫做时间演化算符，通常记为
\begin{equation}
U(t) = e^{-\frac{i}{\hbar} H t }.
\end{equation}
一般地，时间演化算符将某一时刻的态矢与另一时刻的态矢联系起来：
\begin{equation}
|\psi(t) = U(t-t_0)\rangle |\psi(t_0) \rangle.
\end{equation}


量子力学的基本假设之五：全同粒子体系

描写全同粒子系统的态矢对于任意一对粒子的交换是对称的或者反对称的。设有$N$个全同粒子的系统，其态矢可表示为$|\psi(1,2, \cdots, i,\cdots, j, \cdots, N\rangle$。如果交换粒子$i$和$j$，则交换后的态矢为$|\psi(1,2, \cdots, j,\cdots, i, \cdots, N\rangle$。量子力学中假设只有如下两种情况：
\begin{equation}
|\psi(1,2, \cdots, j,\cdots, i, \cdots, N\rangle=
|\psi(1,2, \cdots, i,\cdots, j, \cdots, N\rangle,
\end{equation}
\begin{equation}
|\psi(1,2, \cdots, j,\cdots, i, \cdots, N\rangle=-
|\psi(1,2, \cdots, i,\cdots, j, \cdots, N\rangle.
\end{equation}
满足第一个条件的系统中的粒子叫做Bose子；满足第二个条件的系统中的粒子叫做Fermi子。这个假设反映了全同粒子系统中各个粒子的不可分辨性。


\section{坐标表象的量子力学}

从量子力学的基本原理可知，一个系统的状态和物理量分别是某个Hilbert空间的矢量和算符。这是很抽象的。我们知道，如果在Hilbert空间中选取一个基，态矢就变成了一个列矩阵，而物理量就变成了一个方阵。矩阵比较具体，通常便于计算。在量子力学中，选定一个基被称为选定一个表象（representation），其意义就是让抽象的态矢和算符表示（represent）为具体的矩阵，从而方便计算。

因为基的选取是可以不止一个（实际上有无穷多个），那么同一个问题中表象的选取也就不唯一。如果选择一个物理量$A$的本征态的集合为基，那就称相应的表象为$A$表象。例如，如果$A$是坐标，那么相应的表象就叫坐标表象；如果$A$是动量，那么相应的表象就叫动量表象。类似地，还可以选取能量表象、角动量表象、等等。可以证明：一个物理量在自己的表象中是对角化的；见下面的练习。

\vspace{0.5cm}
\hrule
\hrule
练习。证明：在$A$表象，$A$是一个对角矩阵。
\hrule
\hrule
\vspace{0.5cm}

在一个问题中选择哪个表象要看哪个表象更适合解决问题。有时候，我们还选取两个或多个表象，并在不同的表象之间进行变换。具体内容后面会学习。

在经典物理中，我们经常在坐标空间讨论问题。在量子力学中，也是经常选取坐标表象。为简单起见，我们先讨论一维空间，在适当的时候再推广到三维空间。
...
在坐标表象中，基矢量具有确定的坐标值，可以记为$|\psi_x\rangle$，它代表一个具有确定坐标$x$的态矢。于是，在坐标表象中，坐标算符的本征方程为：
\begin{equation}
x|\psi_x\rangle = x|\psi_x\rangle.
\end{equation}
我们还可以将记号简化。我们完全可以将$|\psi_x\rangle$简写为$|x\rangle$而不损失任何信息，因为它们都可以用来表示一个具有确定坐标值$x$的态矢。这样简化之后，我们有
\begin{equation}
x|x\rangle = x|x\rangle.
\end{equation}
这个记号有个问题，那就是算符（等号左边的态矢前面的$x$）、本征值（等号右边的态矢前面的$x$）以及本征矢量（等号两边的态矢符号里面的$x$）全部都用同一个字母$x$表示。初学者看了可能晕晕的，但老手已经习惯这样的写法了。在很多教科书中，通常将算符的字母头上加一个帽子，以区别于相应的本征值。例如，可以将坐标算符写为$\hat{x}$，从而有
\begin{equation}
\hat{x}|x\rangle = x|x\rangle.
\end{equation}
我不喜欢将算符加个帽子，因为它的存在会让很多公式变得很不清爽。我相信聪明的人总能分清一个字母在一个特定的场合代表什么意思。


我们知道，在经典力学中，坐标可以取连续的值。那么在量子力学中呢？要回答这个问题，我们考察一个由动量构造出来的算符：
\begin{equation}
T(a) = e^{-\frac{i}{\hbar} p a}.
\end{equation}
其中，$p$是动量算符，$a$是任意的实数。从量纲上可以判断，$a$具有坐标的量纲。
可以证明，$T(a)|x\rangle$依然是坐标算符的本征态，本征值为$x+a$:
\begin{equation}
xT(a)|x\rangle = (x+a)T(a)|x\rangle.
\end{equation}

\vspace{0.5cm}
\hrule
\hrule
练习。利用坐标和动量算符的对易关系证明上式。
\hrule
\hrule
\vspace{0.5cm}

既然态矢$T(a)|x\rangle$对应于本征值$x+a$，那么就有
\begin{equation}
T(a)|x\rangle \propto |x + a\rangle.
\end{equation}
如果我们要求$|x\rangle$和$|x+a\rangle$都是基矢量的话，它们就有相同的归一化性质，故上式的比例系数顶多是一个相因子。一个态的整体相因子是不重要的，不妨将它取为1，即
\begin{equation}
T(a)|x\rangle = |x + a\rangle.
\end{equation}
所以算符$T(a)$表现为一个平移算符，它将粒子的坐标平移$a$。这里最关键的结果是：$a$是任意实数。据此可以判断，在量子力学中，坐标也是可以连续取值的。这就回答了上面提出的问题。

既然坐标可以连续取值，那么坐标表象的Hilbert空间就是一个连续空间，即函数空间。我们可以将函数空间的所有数学结论都拿过来应用。下面叙述几个重要的结果。
...
\end{equation}
任意的态矢$|\psi\rangle$在此表象中的表示为
\begin{equation}
|\psi\rangle = I |\psi\rangle =
\int dx |x\rangle \langle x| \psi\rangle,
\end{equation}
其分量$\langle x| \psi\rangle$表现为一个复值函数：
\begin{equation}
\psi(x) \equiv \langle x| \psi\rangle.
\end{equation}
这个函数叫做波函数（wave function），其来由我们稍后再说。
态矢$|\psi\rangle$和$|\phi\rangle$的内积在此表象中的表示为
\begin{equation}
\langle \psi | \phi \rangle =
\int dx \langle \psi | x \rangle \langle x| \phi\rangle =
\int dx  \psi(x)^{\ast} \phi(x).
\end{equation}
特殊地，态矢$|\psi\rangle$的模方为
\begin{equation}
\langle \psi | \psi \rangle =
\int dx  \psi(x)^{\ast} \psi(x) =
\int dx  |\psi(x)|^2.
\end{equation}
如果态矢$|\psi\rangle$是归一化的，那么波函数就满足如下归一化条件
\begin{equation}
\int dx  |\psi(x)|^2 = 1.
\end{equation}
可见，波函数的模方$|\psi(x)|^2$的物理意义是一个概率密度：乘积$|\psi(x)|^2dx$代表系统（即粒子）处于坐标范围
$(x,d+dx)$的概率。波函数的归一化条件说的是，粒子总会出现在空间中的某处。

在进一步讨论之前，我们将前面的讨论推广至三维空间。考虑直角坐标系$(x,y,z)$，其中的坐标矢量表示为$\vect{x}$,则只要将上面所有的的$x$换成$\vect{x}$，$x'$换成$\vect{x}'$即可。其中要注意的是，$d\vect{x}=dxdydz$以及$ \delta(\vect{x}-\vect{x}') = \delta(x-x')\delta(y-y')\delta(z-z')$。


\subsection{坐标表象中的算符}

在数学准备中我们学过，算符对矢量的作用$A|\psi\rangle$在函数空间表示为算符对波函数的作用$A\psi(\vect{x})$
例如，坐标算符对矢量的作用$\vect{x}|\psi\rangle$在坐标表象中就变成$\vect{x}\psi(\vect{x})$，即用函数$f(\vect{x})=\vect{x}$乘以函数$\psi(\vect{x})$。那么动量算符在坐标表象中表示成什么呢？虽然可以一步一步地推导，但我更愿意直接给出结果，然后让你去证明这个结果是对的：
\begin{equation}
\vect{p} = -i\hbar \nabla.
\end{equation}
说这个结果是对的，就是说它满足如下对易关系：
\begin{equation}
...
\hrule
\hrule
练习。通过将上式两边作用于任意波函数来证明，上式是恒成立的。
\hrule
\hrule
\vspace{0.5cm}

知道了动量的表示，也就知道了其它常用物理量的表示。例如，动能在坐标表象中的形式为：
\begin{equation}
T = \frac{\vect{p}^2}{2m} = -\frac{\hbar^2 }{2m}\nabla^2.
\end{equation}
由于坐标在坐标表象中的表示就是变量$\vect{x}$本身，而势能又是坐标的函数，故势能在坐标表象中的表示就是普通的函数。结合势能和动能，就得到Hamilton量的表示
\begin{equation}
H = -\frac{\hbar^2 }{2m}\nabla^2 + U(\vect{x}).
\end{equation}
于是，坐标表象中的定态Schr\"odinger方程为
\begin{equation}
-\frac{\hbar^2}{2m} \nabla^2 \psi(\vect{x})
+ U(\vect{x})\psi(\vect{x}) = E \psi(\vect{x}).
\end{equation}
其中，$E$是待定的能量本征值。
给波函数加上时间依赖性，就得到了相应的含时Schr\"odinger方程
\begin{equation}
i \hbar \frac{\partial \psi(\vect{x},t)}{\partial t} =
-\frac{\hbar^2}{2m} \nabla^2 \psi(\vect{x},t)
+ U(\vect{x})\psi(\vect{x},t).
\end{equation}



\subsection{Schr\"odinger波动方程}

我们提到过$\psi(\vect{x},t)$是波函数，但还没有给予解释。现在从历史的角度来“推导”Schr\"odinger方程，并将$\psi(\vect{x},t)$解释为一个波函数。

先简要回顾一下历史：
\begin{itemize}
\item 1900年12月，Planck为解释黑体辐射，提出量子假说，即电磁辐射中吸收和发射的能量具有分立（“量子”就是“分立”的意思）的值：
  \begin{equation}
   E_n = n h \nu.
  \end{equation}
  其中，$\nu$是一个特定的辐射频率，$n$只能取整数值，$h$就是Planck常数，它的值可以通过理论与实验的比较得出。这个假说的提出标志着量子物理（还不能叫量子力学）的开端。Planck于1918年因此工作获Nobel奖。
\item 1905年，Einstein运用Planck的量子论成功地解释了光电效应。他提出了光子（photon）的概念：光子就是电磁波（即光）的“量子”。一个频率为$\nu$的光子的能量是
  \begin{equation}
   E = h \nu.
  \end{equation}
该式可以用圆频率
...
改写为
  \begin{equation}
   E = h \frac{\omega}{2\pi} = \hbar \omega .
  \end{equation}
也就是说，上面的电磁辐射能量公式$E_n = n h \nu$ 中的整数$n$就是光子的数目。在光电效应中，电子被光照射后跑出金属，是因为电子被一个个的光子撞击了。如果金属对电子的束缚能是$A$（也叫逸出功），那么跑出来的电子的最大的能量为
\begin{equation}
E_{max} = h\nu - A.
\end{equation}
这就是光电效应的Einstein公式，它后来被Millikan （1916年）的实验完美地证实。而且，Millikan的数据还给出了Planck常数的一个值，与由黑体辐射数据确定的Planck常数的值吻合的很好。Einstein于1921年因此工作获Nobel奖。Millikan则于1923年因此工作以及关于基本电荷的实验工作获Nobel奖。
\item 1906年，Einstein继续将Planck的量子假说发扬光大，应用于固体热容的问题，成功地解释了固体热容随温度的降低而减小的实验结果。固体中的波动也具有量子性，所以后来用“声子”（phonon）来指代固体中的波动。
\item 1923年Compton的实验更进一步地验证了光的粒子性。Compton于1927年因此工作获Nobel奖。
\item 如果说以上实验和理论确立的光了波粒二象性（wave-particle duality），那么关于物质（电子、原子之类的“实物”粒子）的波粒二象性是由学文科（历史）的de Broglie于1924年提出的。他假定任何粒子（无论是光还是物质）的动量$p$和相应的波的波长$\lambda$之间有如下关系：
\begin{equation}
p = \frac{h}{\lambda}.
\end{equation}
上式还可以用波矢大小
...
\vect{p}  = \hbar \vect{k}.
\end{equation}
de Broglie于1929年凭此得了物理的Nobel奖。
\end{itemize}


物质粒子有波动性，那么就需要有一个相应的波函数以及波动方程。如果我们考虑一个具有确定的动量$\vect{p}$ 和能量$E$的自由粒子，它的波函数应该如何表达呢？将粒子看做波动，对应的波矢和圆频率为$\vect{k}=\vect{p}/\hbar$和$\omega=E/\hbar$。类比电磁波的形式，我们猜测该粒子的波函数为如下平面波（plane wave）：
\begin{equation}
\psi(\vect{x},t) = \exp\left[i\left( \vect{k}\cdot\vect{x} - \omega t \right) \right].
\end{equation}
将$\vect{k}$和$\omega$换成$\vect{p}$和$E$，得
\begin{equation}
\psi(\vect{x},t) =
\exp\left[\frac{i}{\hbar}\left( \vect{p}\cdot\vect{x} - E t \right) \right].
\end{equation}
注意到这个波描述的是一个自由粒子，其动量与能量有如下简单的关系：
\begin{equation}
E = \frac{\vect{p}^2}{2m}.
\end{equation}
为了猜出波函数满足的方程，我们将上式两边都乘以波函数：
\begin{equation}
E \exp\left[\frac{i}{\hbar}\left( \vect{p}\cdot\vect{x} - E t \right) \right]= \frac{\vect{p}^2}{2m}
\exp\left[\frac{i}{\hbar}\left( \vect{p}\cdot\vect{x} - E t \right) \right].
\end{equation}
如果还记得有关电磁波的推导的话，就容易看出上式可以由下面的方程给出：
\begin{equation}
i\hbar \frac{\partial}{\partial t} \exp\left[\frac{i}{\hbar}\left( \vect{p}\cdot\vect{x} - E t \right) \right]= \frac{-\hbar^2 }{2m}\nabla^2
\exp\left[\frac{i}{\hbar}\left( \vect{p}\cdot\vect{x} - E t \right) \right].
\end{equation}
以上推导是针对一个平面波的。若假设所有的波都能够用若干平面波展开，则可推知对一般的波函数$\psi(\vect{x},t)$，我们也有类似的方程：
\begin{equation}
i\hbar \frac{\partial}{\partial t} \psi(\vect{x},t)
= \frac{-\hbar^2 }{2m}\nabla^2\psi(\vect{x},t).
\end{equation}
将此式与上一节的公式对比可知，我们确实“推导”出了自由粒子在坐标表象的Schr\"odinger方程，但这个方程还不是一般的Schr\"odinger方程，用处不大。上面的“推导”并不是Schr\"odinger本人最初的方法。下面大致介绍Schr\"odinger本人在1926年用的方法。

我们知道，均匀弦的振动方程为
\begin{equation}
\frac{\partial^2 u(x,t)}{\partial x^2} - \frac{1}{v^2}\frac{\partial^2 u(x,t)}{\partial t^2} =0.
\end{equation}
其中，$u(x,t)$是弦上的$x$处的那一点于$t$时刻在横向（垂直与$x$的方向）的位移，$v$是振动在弦中传播的速度（即振动波的波速）。推导可参考梁昆淼的《数学物理方法》第七章。该方程的通解为
\begin{equation}
u(x,t) = \psi(x) \sin(\omega t).
\end{equation}
其中，$\psi(x)$只与空间坐标$x$有关，时间因子$\sin(\omega t)$中的$\omega$代表振动的角频率。将此解代入上述波动方程，得
\begin{equation}
\frac{\partial^2 \psi(x)}{\partial x^2} + \frac{\omega^2}{v^2}\psi(x)=0.
\end{equation}
利用如下关系式（$\nu$是频率，$\lambda$是波长，$k$ 是波矢）
\begin{equation}
\frac{\omega}{v}=\frac{2\pi \nu}{v} = \frac{2\pi}{\lambda} = k,
\end{equation}
我们有
\begin{equation}
...
上面考虑的是弦振动。现在考虑一个微观粒子。根据de Broglie的公式，一个总能量为$E$，势能为$U(x)$的粒子的波矢为
\begin{equation}
k = \frac{p}{\hbar} = \frac{\sqrt{2m(E-U(x))}}{\hbar}.
\end{equation}
Schr\"odinger假设，如果把粒子看作空间中的波$\psi(x)$的话，这个波满足上述弦振动方程，其中的波矢为de Broglie波矢：
\begin{equation}
\frac{\partial^2 \psi(x)}{\partial x^2} + \left(\frac{\sqrt{2m(E-U(x))}}{\hbar}\right)^2 \psi(x)=0.
\end{equation}
稍加整理就得到不含时的一维空间的Schr\"odinger方程：
\begin{equation}
 - \frac{\hbar^2}{2m} \frac{d^2}{dx^2}\psi(x) + U(x) \psi(x) = E \psi(x).
\end{equation}
推广到三维空间，就是
\begin{equation}
 - \frac{\hbar^2}{2m} \nabla^2 \psi(\vect{x}) + U(\vect{x}) \psi(\vect{x}) = E \psi(\vect{x}).
\end{equation}
以上基本上就是Schr\"odinger提出Schr\"odinger方程的过程。现在，我们就知道坐标表象的态函数$\psi(\vect{x},t)$为什么叫做波函数了，因为它满足一个波动方程。相应地，坐标表象的Schr\"odinger方程就叫做波动方程。
...
在考虑物理量的期望值之前，先考察一下波函数的归一化性质。我们知道，在坐标表象中，波函数的归一化表示为
\begin{equation}
\int d\vect{x} \psi(\vect{x}, t)^{\ast}  \psi(\vect{x}, t) = 1.
\end{equation}
这就是说，$\psi(\vect{x}, t)^{\ast}  \psi(\vect{x}, t)d\vect{x}$代表在$\vect{x}$附近的体积元$d\vect{x}$ 内发现粒子的概率。这就是波函数的统计诠释，是Born于1926年提出的。如果这个统计诠释是合理的话，必须要求波函数的归一化性质与时间无关：如果波函数的归一化依赖于时间的话，那么波动方程就没有唯一的解了。
...
练习。证明如下等式
\begin{equation}
\frac{\partial}{\partial t} \rho(\vect{x}, t) + \nabla \cdot \vect{j}(\vect{x}, t) = 0.
\end{equation}
其中
\begin{equation}
\rho(\vect{x}, t) = \psi(\vect{x}, t)^{\ast}  \psi(\vect{x}, t),
\end{equation}
就是概率密度，而
\begin{equation}
\vect{j}(\vect{x}, t) =
\frac{i\hbar}{2m}\left[ (\nabla \psi^{\ast}) \psi - \psi^{\ast} (\nabla \psi)\right],
\end{equation}
叫做概率流密度。
\hrule
\hrule
...
上面练习中的结论反映的是概率守恒，它类似于电动力学中的电荷守恒。根据该结果，再利用积分可得
\begin{equation}
\frac{\partial}{\partial t} \int_{\Omega} d\vect{x} \rho(\vect{x}, t) =
- \int_{\Omega} d\vect{x}\nabla \cdot \vect{j}(\vect{x}, t)
= -\oint_{S} d\vect{a} \cdot \vect{j}(\vect{x}, t).
\end{equation}
从物理的考虑可知，上式中的闭曲面积分一定等于零。所以，波函数的归一化是与时间无关的。注意，概率流密度是实数，它也可以写成
\begin{equation}
\vect{j}(\vect{x}, t) = 2\text{Re}
...
\hrule
\hrule
练习。证明：平面波的概率流密度等于相应的粒子的速度，即
\begin{equation}
\vect{j}=\frac{\vect{p}}{m}.
\end{equation}
\hrule
\hrule
\vspace{0.5cm}

下面，我们可以放心地考虑物理量的期望值了。回顾函数空间的内容可知，对于一般的波函数$\psi(\vect{x}, t)$，任意物理量$A$的期望值为
\begin{equation}
\langle A \rangle = \int d\vect{x} \psi(\vect{x}, t)^{\ast} A \psi(\vect{x}, t).
\end{equation}
其中，物理量$A$要理解为算符函数$A\left(\vect{x}, -i\hbar \nabla \right)$。例如，坐标的期望值为
\begin{equation}
\langle \vect{x} \rangle
= \int d\vect{x} \psi(\vect{x}, t)^{\ast} \vect{x} \psi(\vect{x}, t).
\end{equation}
注意：该式要按如下方式理解为三个分量方程
\begin{equation}
\langle x \rangle
= \int d\vect{x} \psi(\vect{x}, t)^{\ast} x \psi(\vect{x}, t),
\end{equation}
\begin{equation}
\langle y \rangle
= \int d\vect{x} \psi(\vect{x}, t)^{\ast} y \psi(\vect{x}, t),
\end{equation}
\begin{equation}
\langle z \rangle
= \int d\vect{x} \psi(\vect{x}, t)^{\ast} z \psi(\vect{x}, t).
\end{equation}
动量的期望值为
\begin{equation}
\langle \vect{p} \rangle
= \int d\vect{x} \psi(\vect{x}, t)^{\ast} \left(-i\hbar \nabla \right) \psi(\vect{x}, t).
\end{equation}
我们同样需要将该式理解为三个分量方程。能量的期望值
\begin{equation}
\langle E \rangle
= \int d\vect{x} \psi(\vect{x}, t)^{\ast}
\left(-\frac{\hbar^2}{2m} \nabla^2 + U(\vect{x}) \right)
\psi(\vect{x}, t).
\end{equation}
读者不难分别写出势能和动能的期望值。
