/*****************************************************************************80
    ...
    This is the source code of MD_RDF, which demonstrates how to calculate the
    radial distribution function (RDF) in molecular dynamics (MD) simulation.

    MD_RDF is a free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    MD_RDF is distributed in the hope that it will be useful,
...
The units used in this program are as follows:
    ...
    (2) Some derived units are
        (A) Mechanics
            ...
            -- force: eV A^(-1)
        (B) Thermodynamics
            Boltzmann constant: K_B ~ 0.8617343 * 10^(-4) eV K^(-1)
------------------------------------------------------------------------------*/
...
#define K_B                   8.617343e-5 // Boltzmann's constant in my unit
#define TIME_UNIT_CONVERSION  1.018051e+1 // fs <-> my unit
...
(
    ...
    double *vx, double *vy, double *vz, double *x, double *y, double *z, int flag
)
{
...
int main(int argc, char *argv[])
{
    ...
    int Ns = 100;      // sampling interval
    double time_step = 5.0 / TIME_UNIT_CONVERSION; // time step
    
    // The state point corresponds to that in Fig. 6.22 of Haile's book
    double T_0 = 130;          // temperature prescribed (K)
    double ax = 5.60;          // lattice constant in the x direction (A)
    ...
    double rcn = 12.0;     // cutoff distance for neighbor list
    double rcf = 10.0;     // cutoff distance for force
    int neighbor_update_interval = 5; // interval of neighbor list update
    int MN = 256;                     // amximal number of neighbors 
