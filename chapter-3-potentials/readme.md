...
经典势函数，简称经典势，是相对于量子力学势来说的。一个经典多粒子系统的势能可以写为各个粒子坐标的函数。文献中也将该势函数成为力场（force field）或者势能面（potential energy surface）。
...
经验势是用具有物理或者化学意义的数学函数表达的经典势。其中的参数一般通过拟合实验结果来确定。迄今已有众多的经验势多体势，其中最为广泛使用的当属 EAM 势和 Tersoff 势。
